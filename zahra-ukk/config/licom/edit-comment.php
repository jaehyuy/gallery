<?php
    ...
    $sql=mysqli_query($c,"SELECT * FROM komentarfoto WHERE komentarid= '".$_GET['komentarid']."'");
...
<html lang="en">
    <head>
        ...
        <title>Edit Comment - Gallery</title>
                    ...
                    <div class="card transparent-card">
                        ...
                        <div class="card-body text-center">
                            <form action="" method="POST">
                                <input type="hidden" name="komentarid" value="<?php echo $d['komentarid'] ?>">
                                <div class="form-group mt-2">
                                    <label class="form-label">New Comments</label>
                                    <input type="text" name="isikomen" value="<?php echo $d['isikomen'] ?>" class="form-control text-center">
                                </div>
                            ...
                            <?php
                                if(isset($_POST['edit1'])) {
                                    $fotoid = $_GET['fotoid']; // Ambil nilai fotoid dari URL
                                    $komentarid = $_POST['komentarid'];
                                    $isikomen = $_POST['isikomen'];
                                    $tglkomen = date('Y-m-d');
                                    $userid = $_SESSION['userid'];
                                    $sql = mysqli_query($c,"UPDATE komentarfoto SET isikomen='$isikomen', tglkomen='$tglkomen' WHERE komentarid='$komentarid' AND fotoid='$fotoid'");
